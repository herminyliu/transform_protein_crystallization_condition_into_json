...
# 初始化vLLM
llm = LLM(model="deepseek-r1-distill-llama-8b")  # 替换为你的模型名称
...
def llm_process(original_sent):
    ...
    sampling_params = SamplingParams(temperature=0.7, top_p=0.9, max_tokens=200, guided_decoding=guided_decoding_params)
    content = propmt_1 + original_sent
    outputs = llm.generate(propmts=content, sampling_params=sampling_params)
...
def process_csv(input_csv_path, output_csv_path, is_check, is_refine):
    ...
    extracted_pH_lst, extracted_temperature_lst, extracted_method_lst, extracted_reagents_lst, refined_json_lst = [], [], [], [], []
...
def refineData(json_structure, original_sent):
    # 使用vLLM生成文本
    guided_decoding_params = GuidedDecodingParams(json=json_schema)
    sampling_params = SamplingParams(temperature=0.6, top_p=0.9, max_tokens=2048, guided_decoding=guided_decoding_params)
